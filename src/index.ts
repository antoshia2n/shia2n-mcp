 ...
 *          設定の追加は無い（SUPABASE_URL と SUPABASE_SERVICE_ROLE_KEY は既存）。
 * v0.57.0：学ぶくんの道具を直した（2026-08-24・同じ日の 2 回目）
 *          ① 日付を題名から外した。題名は題名だけにし、日付は「説明」の先頭と
 *            本文の先頭に置く。学ぶくんに日付の欄が無く、一覧の札は題名と
 *            その下の説明の 2 行で出るため、説明の先頭に置くと別の行に出る。
 *          ② 同じものかどうかの見分けを、題名から日付に変えた。題名を後から
 *            直しても同じ 1 件として扱われ、二重にならない。
 *          ③ 1 回の呼び出しで最大 50 本まで入るようにした（142 本を 142 回に
 *            しないため）。渡された行に同じ日付が 2 つあれば 1 行も書かずに止まる。
 ...
 *          題名・動画の番号・マインドマップの住所と、1 行ごと／棚ごとの指紋
 *          （題名の文字数と FNV-1a の要約値）を返す。
 *          なぜ指紋か：題名そのものを突き合わせるには、返ってきた文字を
 *          もう一度こちらの応答へ書き写す工程が入る。その二度目の書き写しで
 *          無意識に直してしまうと、差 0 件と出て誤りが隠れる（2026-08-24 に
 *          題名の取り違えが 2 回続き、どちらも目で拾っていた）。
 *          数で比べれば 1 文字違っても必ずずれる。
 *          マインドマップの住所を一緒に返すのは、第 2 便（カレッジ）で
 *          新しく入る本数を、すでに入っている分を引いて機械で出すため。
