...
export function registerManabuTools(server: McpServer, env: Env): void {
  server.tool(
    ...
    "YouTube URLからレッスンページを自動作成する。動画タイトル・文字起こし全文・構造化サマリ（章立て・主要ポイント）を生成し、mn_lessonsテーブルに保存する。字幕付き・30分以下の動画が対象。",
    {
      youtube_url: z
        .string()
    ...
    async ({ youtube_url, course_id }) => {
      ...
      // 4. 文字起こし取得
      const transcript = await mnFetchTranscript(videoId);
      if (!transcript) {
        return {
          content: [
            {
              type: "text",
              text: JSON.stringify({
                error: "この動画には字幕がありません。字幕付きの動画を使用してください（自動字幕でも可）。",
                video_title: title,
                hint: "YouTubeで動画を開き「字幕」アイコンが表示されるか確認してください。",
              }),
            },
          ],
        };
      ...
      // 5. 構造化サマリ生成
      const summaryJson = await mnGenerateSummary(title, transcript, env.ANTHROPIC_API_KEY);
      ...
      if (!insertRes.ok) {
        ...
        return {
          content: [
            {
              type: "text",
              text: JSON.stringify({ error: `Supabase 保存失敗: ${errText}` }),
            },
          ],
        };
      }
